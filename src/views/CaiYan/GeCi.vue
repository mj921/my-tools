...
const key = ref(localStorage.getItem('caiyan-geci'));
...
const reset = async () => {
  const res = await request<{ title: string; content: { paragraphs: string[][] } }>({
    url: `/caiyan/geci/${key.value}.json`,
  });
...
const getData = async (random = false) => {
  if (keys.value.length === 0) {
    keys.value = await request<string[]>({ url: '/caiyan/geci/sum.json' });
  }
  if (!key.value || random) {
    key.value = keys.value[Math.floor(Math.random() * keys.value.length)];
    localStorage.setItem('caiyan-geci', key.value);
  }
  await reset();
};
getData();
