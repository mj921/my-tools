<template>
  <div class="ds-group">
    <dt class="ds-group-header">群聊 <PlusIcon @click="addGroup" /></dt>
    <div class="ds-group-list">
      <dl
        :class="[
          'ds-group-item',
          { 'ds-group-item--active': group.key.toString() === route.params.chatKey },
        ]"
    ...
    <div class="deepseek-mask" v-show="visible">
      <div class="ds-group-modal">
        <div class="ds-group-modal-header">群聊</div>
...
const save = () => {
  if (editKey.value) {
    dbtool.updateGroupChat({ key: editKey.value, name: groupName.value }).then((res) => {
      if (!res?.success) {
    ...
    dbtool.addGroupChat(groupName.value).then((res) => {
      if (!res?.success) {
  ...
  visible.value = false;
  dbtool.getGroupChatList().then((res) => {
    groupList.value = res;
  });
};
const addGroup = () => {
...
const delGroup = () => {
  if (menuGroup.value) {
    ...
    modal
      ...
      .then(() => {
        dbtool.deleteGroupChat(editKey.value).then((res) => {
          if (!res?.success) {
...
const selectGroup = (chatKey: number) => {
  router.push({
